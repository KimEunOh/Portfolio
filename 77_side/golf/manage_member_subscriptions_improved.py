...
print("=" * 70)
print("회원별 이용권 정보 관리 시스템 (순수이름 기반)")
print("=" * 70)

# 1. 개선된 파일 읽기
print("\n1. 순수이름 기반 회원 데이터 파일 읽기")
improved_file = "2024.01.01 ~2024.12.31 종료된 회원_순수이름.xlsx"
active_file = "현재 이용중인 회원_순수이름.xlsx"

# 파일 존재 확인
files_exist = True
for file_path in [improved_file, active_file]:
...
if not files_exist:
    print(
        """
필요한 파일이 존재하지 않습니다. 먼저 extract_member_name.py 스크립트를 실행하여 
순수이름 기반의 회원 파일을 생성해주세요.
    """
    )
    exit(1)
...
# 순수이름 추출 추가
subscriptions_df["순수이름"] = subscriptions_df["이름"].apply(extract_base_name)
...
# 저장할 파일명
subscription_file = "회원별_이용권_정보_순수이름.xlsx"
summary_file = "회원별_이용권_요약_순수이름.xlsx"
...
# 이름으로 검색 (박민규, 박민규(상무) 모두 검색됨)
result2 = search_member_subscriptions(name='박민규')
